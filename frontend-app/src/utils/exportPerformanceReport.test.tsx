...
const rows: ExportPerformanceShipment[] = [
  { id: 1, sheetNo: 'S1', invoiceDate: '2026-09-10', styleNos: ['BF1', 'LB1'], brands: ['라베', '빈폴'], lineCount: 3, qtyByUnit: { EA: 10, YD: 150 }, amount: 1530, linesWithoutAmount: 0 },
  { id: 2, sheetNo: null, invoiceDate: null, styleNos: [], brands: [], lineCount: 1, qtyByUnit: {}, amount: 0, linesWithoutAmount: 1 },
];
...
describe('수출 실적표 화면 유틸 (PR-119)', () => {
  ...
  it('엑셀은 서버가 집계한 문서 목록과 그대로 일치한다(재파싱)', async () => {
    ...
    expect(p[0]).toEqual(['INVOICE 일자', '문서번호', '스타일번호', '브랜드', '수량', '금액', '단가 미정 라인 수']);
    expect(p[1]).toEqual(['2026-09-10', 'S1', 'BF1, LB1', '라베, 빈폴', '150 YD / 10 EA', 1530, 0]);
    expect(p[2]).toEqual(['', '', '', '', '-', 0, 1]);
  });

  it('데이터 유무 판정', () => {
    expect(hasPerformanceData(null)).toBe(false);
    expect(hasPerformanceData({ totals: { shipmentCount: 0, lineCount: 0, qtyByUnit: {}, amount: 0, linesWithoutAmount: 0 }, byBrand: [], shipments: [], excludedNotFinalized: 0 })).toBe(false);
    expect(hasPerformanceData({ totals: { shipmentCount: 1, lineCount: 1, qtyByUnit: {}, amount: 0, linesWithoutAmount: 0 }, byBrand: [], shipments: rows, excludedNotFinalized: 0 })).toBe(true);
  });
});
