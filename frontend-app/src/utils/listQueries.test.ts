...
import { buildItemsQuery, buildWorkOrdersQuery, fetchItemPage, fetchWorkOrderPage } from './listQueries';
...
describe('listQueries — 목록 조회 파라미터 / 페이지 재조회 (PR-128)', () => {
  ...
  describe('buildWorkOrdersQuery', () => {
    it('page와 limit 10을 항상 보내고, 빈 상태/검색어는 뺀다(All → status 없음)', () => {
      expect(buildWorkOrdersQuery({ page: 1, status: '', keyword: '  ' })).toEqual({ page: 1, limit: 10, status: undefined, keyword: undefined });
    });
    it('상태와 검색어(공백 제거)를 그대로 싣는다', () => {
      expect(buildWorkOrdersQuery({ page: 3, status: 'COMPLETED', keyword: ' 셔츠 ' })).toEqual({ page: 3, limit: 10, status: 'COMPLETED', keyword: '셔츠' });
    });
  });
  ...
  describe('fetchWorkOrderPage — 페이지 이동 시 올바른 page로 재조회', () => {
    it('page를 바꿔 다시 부르면 그 page가 API 파라미터로 나가고 meta가 돌아온다', async () => {
      ...
      expect(getWorkOrders).toHaveBeenNthCalledWith(1, { page: 1, limit: 10, status: 'PENDING', keyword: undefined });
      expect(getWorkOrders).toHaveBeenNthCalledWith(2, { page: 2, limit: 10, status: 'PENDING', keyword: undefined });
