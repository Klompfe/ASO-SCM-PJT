...
const data = [
  mk(1, 'BF6821C52', 'PENDING_CLEARANCE', [{ qty: 10, amount: '100.5' }, { qty: 5, amount: 50 }], { brand: 'BF' }),
  mk(2, 'LB69SLM101A', 'CLEARED', [{ qty: 20, amount: 200 }], { brand: 'LB', clearedAt: '2026-07-20T00:00:00.000Z' }),
...
describe('수입/통관 현황표 (PR-114)', () => {
  ...
  it('엑셀 워크북은 화면 데이터와 일치한다(재파싱 비교, 브랜드 포함)', async () => {
    ...
    expect(p[0]).toEqual(['스타일번호', '브랜드', 'INVOICE 번호', 'INVOICE 일자', '상태', '통관일', '수량', '금액']);
    expect(p[1]).toEqual(['BF6821C52', 'BF', 'INV-1', '2026-07-18', '통관대기', '', 15, 150.5]);
    expect(p[2]).toEqual(['LB69SLM101A', 'LB', 'INV-2', '2026-07-18', '통관완료', '2026-07-20', 20, 200]);
  ...
  it('인쇄용 표는 모든 행과 상태 라벨을 렌더링한다', () => {
    ...
    for (const t of ['BF6821C52', 'LB69SLM101A', 'DR0H6D01', '통관대기', '통관완료']) expect(html).toContain(t);
  });
});
