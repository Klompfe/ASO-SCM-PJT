...
describe('CreateBuyerDto — email 선택 입력 (PR-149)', () => {
  ...
  it('email을 아예 안 보내면(undefined) 통과한다', async () => {
    const dto = plainToInstance(CreateBuyerDto, { name: '(주) 미도컴퍼니' });
    const errors = await validate(dto);
    expect(emailErrors(errors)).toEqual({});
  });

  it('email이 빈 문자열이어도(프론트가 실제로 보내는 값) 통과한다', async () => {
    const dto = plainToInstance(CreateBuyerDto, { name: '(주) 미도컴퍼니', email: '' });
    const errors = await validate(dto);
    expect(emailErrors(errors)).toEqual({});
  });
  ...
  it('email 형식이 잘못되면 여전히 거절된다', async () => {
    for (const bad of ['abc', 'abc@']) {
      const dto = plainToInstance(CreateBuyerDto, { name: '(주) 미도컴퍼니', email: bad });
      const errors = await validate(dto);
  ...
  it('유효한 email은 통과하고 값이 그대로 유지된다', async () => {
    const dto = plainToInstance(CreateBuyerDto, { name: '(주) 미도컴퍼니', email: 'contact@buyer.com' });
    const errors = await validate(dto);
    expect(emailErrors(errors)).toEqual({});
    expect(dto.email).toBe('contact@buyer.com');
  });

  // PartialType(CreateBuyerDto)이므로 @Transform이 그대로 상속되는지 확인.
  it('UpdateBuyerDto도 email 빈 문자열을 통과시킨다', async () => {
    const dto = plainToInstance(UpdateBuyerDto, { email: '' });
    const errors = await validate(dto);
    expect(emailErrors(errors)).toEqual({});
  });

  it('UpdateBuyerDto도 잘못된 email 형식은 거절한다', async () => {
    const dto = plainToInstance(UpdateBuyerDto, { email: 'not-an-email' });
    const errors = await validate(dto);
    expect(emailErrors(errors).isEmail).toBe('유효한 이메일 형식이 아닙니다.');
  });
});
